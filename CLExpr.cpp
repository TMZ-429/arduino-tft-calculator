...
namespace
{
    ...
    const double kR = 8.31446261815324;
...
namespace
{
    ...
    double EvalNumber(cEvalContext* context)
    {
        ...
        if (isalpha(*context->s))
        {
            ...
            if (Token(s0, s1, "cot"))      return 1.0 / tan(EvalParentheses(context));
            if (Token(s0, s1, "sec"))      return 1.0 / cos(EvalParentheses(context))
